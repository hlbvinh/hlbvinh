...
dag = DAG(
    "user_models", catchup=False, default_args=default_args, schedule_interval=interval
)

t1 = BashOperator(
    task_id="user_sample_generator",
    bash_command=(
        f"{pex_path} user_sample_generator --config {config_path} "
...
t2 = BashOperator(
    task_id="comfort_model_training",
    bash_command=(
        f"{pex_path} comfort_model --config {config_path} " f"--log_directory {log_dir}"
    ),
    dag=dag,
)
